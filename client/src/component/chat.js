    ...
    const fetchChats = async () => {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                setChats(data);
                // Find the chat where chat.chatname === activeChat and chat.isGroupChat === false
                const selectedChat = data.find(chat => chat.chatName === activeChat && !chat.isGroupChat);

                // Set the selected chat if found
                if (selectedChat) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <button
                    className="flex absolute gap-2 items-center bottom-10 left-40 bg-yellow-400 px-4 py-2 rounded-lg hover:bg-yellow-500"
